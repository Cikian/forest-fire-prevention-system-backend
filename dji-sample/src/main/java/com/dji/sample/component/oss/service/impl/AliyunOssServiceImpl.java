...
public class AliyunOssServiceImpl implements IOssService {
    ...
    public CredentialsToken getCredentials() {
        ...
        try {
            DefaultProfile profile = DefaultProfile.getProfile(
                    DjiOssConfiguration.region, DjiOssConfiguration.accessKey, DjiOssConfiguration.secretKey);
            IAcsClient client = new DefaultAcsClient(profile);

            AssumeRoleRequest request = new AssumeRoleRequest();
            request.setDurationSeconds(DjiOssConfiguration.expire);
            request.setRoleArn(DjiOssConfiguration.roleArn);
            request.setRoleSessionName(DjiOssConfiguration.roleSessionName);

            AssumeRoleResponse.Credentials response = client.getAcsResponse(request).getCredentials();
            return new CredentialsToken(response.getAccessKeyId(), response.getAccessKeySecret(), response.getSecurityToken(), DjiOssConfiguration.expire);
    ...
    public URL getObjectUrl(String bucket, String objectKey) {
        ...
        return ossClient.generatePresignedUrl(bucket, objectKey,
                new Date(System.currentTimeMillis() + DjiOssConfiguration.expire * 1000));
    }
    ...
    public void createClient() {
        ...
        this.ossClient = new OSSClientBuilder()
                .build(DjiOssConfiguration.endpoint, DjiOssConfiguration.accessKey, DjiOssConfiguration.secretKey);
    }
}
