...
public class MqttMessageChannel {
    ...
    @Bean(name = ChannelName.INBOUND_EVENTS_AIRSENSE_WARNING)
    public MessageChannel inboundEventsAirsenseWarning() {
        return new DirectChannel();
    }

    @Bean(name = ChannelName.INBOUND_EVENTS_FLIGHTTASK_PROGRESS)
    public MessageChannel flighttaskProgressChannel() {
        return new DirectChannel();
    }
