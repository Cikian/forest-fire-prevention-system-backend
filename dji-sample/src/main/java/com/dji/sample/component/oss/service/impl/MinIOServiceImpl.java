...
public class MinIOServiceImpl implements IOssService {
    ...
    public CredentialsToken getCredentials() {
        try {
            AssumeRoleProvider provider = new AssumeRoleProvider(DjiOssConfiguration.endpoint, DjiOssConfiguration.accessKey,
                    DjiOssConfiguration.secretKey, Math.toIntExact(DjiOssConfiguration.expire),
                    null, DjiOssConfiguration.region, null, null, null, null);
            Credentials credential = provider.fetch();
            return new CredentialsToken(credential.accessKey(), credential.secretKey(), credential.sessionToken(), DjiOssConfiguration.expire);
            ...
            return new URL(
                    client.getPresignedObjectUrl(
                                    GetPresignedObjectUrlArgs.builder()
                                            ...
                                            .bucket(bucket)
                                            .object(objectKey)
                                            .expiry(Math.toIntExact(DjiOssConfiguration.expire))
                                            .build()));
    ...
    public void createClient() {
        ...
        this.client = MinioClient.builder()
                .endpoint(DjiOssConfiguration.endpoint)
                .credentials(DjiOssConfiguration.accessKey, DjiOssConfiguration.secretKey)
                .region(DjiOssConfiguration.region)
                .build();
    }
}
