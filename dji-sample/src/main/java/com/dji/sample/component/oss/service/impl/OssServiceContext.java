...
public class OssServiceContext {
    ...
    @Autowired
    public OssServiceContext(List<IOssService> ossServices, DjiOssConfiguration configuration) {
        if (!DjiOssConfiguration.enable) {
            return;
        }
        this.ossService = ossServices.stream()
                .filter(ossService -> ossService.getOssType() == DjiOssConfiguration.provider)
                .findFirst()
